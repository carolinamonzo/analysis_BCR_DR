...
def plot_CDR3_biorep(df):
    ...
    if run_type != "dry":
        plt.savefig("{}{}_cdr3_biogroup_{}.pdf".format(path, organ, new_day))
    else:
        plt.show()


plot_CDR3_biorep(ile)
...
# {{{
if run_type != "dry":
    dens_samp.to_csv("../../analysis/results_tables/CDR3_{}_{}.csv".format(organ, new_day), index = False)

dens_samp.head()
# }}}

# Bring metadata to be able to group
...
df_dens_biogroup = df_dens_mouse.groupby("biorep").agg({"Mu":np.mean, "Sigma":np.mean}).reset_index()
...
g = g.map(sns.distplot, "CDR3aa_length", hist = False, kde = False, fit = norm, fit_kws={"linewidth":3})
...
if run_type != "dry":
    plt.savefig("{}{}_cdr3_allgrid_{}.pdf".format(path, organ, new_day))
...
anova_lm(ols("Num_significant ~ Age * Diet + Age + Diet", data = ratios_df[ratios_df["Diet"].isin(["AL lifelong", "DR lifelong"])]).fit(), typ=2)
...
if run_type != "dry":
    plt.savefig("{}{}_MuCDR3_boxplot_{}.pdf".format(path, organ, new_day))
else:
