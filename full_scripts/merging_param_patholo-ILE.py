...
# {{{
AL5 = pd.read_csv(path_ori + 'alpha_values_5AL_{}.tsv'.format(organ), sep = " ")
DR5 = pd.read_csv(path_ori + 'alpha_values_5DR_{}.tsv'.format(organ), sep = " ")
AL16 = pd.read_csv(path_ori + 'alpha_values_16AL_{}.tsv'.format(organ), sep = " ")
AL20 = pd.read_csv(path_ori + 'alpha_values_20AL_{}.tsv'.format(organ), sep = " ")
DR20 = pd.read_csv(path_ori + 'alpha_values_20DR_{}.tsv'.format(organ), sep = " ")
AL24 = pd.read_csv(path_ori + 'alpha_values_24AL_{}.tsv'.format(organ), sep = " ")
DR24 = pd.read_csv(path_ori + 'alpha_values_24DR_{}.tsv'.format(organ), sep = " ")
ALDR16M20 = pd.read_csv(path_ori + 'alpha_values_20AL_DR16M_{}.tsv'.format(organ), sep = " ")
ALDR16M24 = pd.read_csv(path_ori + 'alpha_values_24AL_DR16M_{}.tsv'.format(organ), sep = " ")
ALDR20M24 = pd.read_csv(path_ori + 'alpha_values_24AL_DR20M_{}.tsv'.format(organ), sep = " ")
...
pert_path_alpha.to_csv("{}/df_tumour_countsPA_alpha_{}.csv".format(path, organ), index = False)
...
def correlation_numtum_alpha(dfa, alpha):
    ...
    plt.tight_layout()
    if run_type != "dry":
        plt.savefig("{}corr_{}_numtum_{}_{}.pdf".format(path, alpha, organ, new_day))
    else:
        plt.show()
...
if run_type != "dry":
    tum_counts_alpha.to_csv("{}/df_tumour_counts_alpha_{}.csv".format(path, organ), index = False)

# {{{
# Generate same dataframe for pert dataframes
pert_path_alpha = pd.merge(all_pert.drop(columns = ["Age", "diet"]), df2.loc[:, ["Animal_ID", "biogroup", "q", "d", "Age", "Diet"]], on = "Animal_ID", how = "inner")
# Save pathology with alpha from spleen for analysis in R (pathology_alpha_stats.Rmd)
pert_path_alpha.to_csv("{}/df_tumtis_alpha_{}.csv".format(path, organ), index = False)
...
# Save pathology with alpha from spleen for analysis in R (pathology_alpha_stats.Rmd)
if run_type != "dry":
    pert_path_alpha.to_csv("{}/df_tumtisPA_alpha_{}.csv".format(path, organ), index = False)
# }}}
...
shm = pd.read_csv(f"../../analysis/results_tables/{organ}_frequency_values_20211106.csv", sep = ";")
...
cdr3 = pd.read_csv("../../analysis/results_tables/CDR3_{}_20220404.csv".format(organ), sep = ",")
cdr3.columns = ["Animal_ID", "cdr3_mu", "cdr3_sigma"]
tum_alpha_p20_shm_cdr3 = pd.merge(tum_alpha_p20_shm, cdr3, on = "Animal_ID", how = "inner")

# ## Read ClassSwitch

cs = pd.read_csv("../../analysis/plots/naiveClassSwitch/{}_NaiveClassw_20220412.csv".format(organ), sep = ";")
...
beta = pd.read_csv("../../analysis/results_tables/RDI_samp_{}.tsv".format(organ), sep = " ")
...
beta[["sample_id", "biogroup"]] = beta["row"].str.split(" ", expand = True)
beta = beta.loc[:, ["sample_id", "value"]]
...
v = pd.read_csv(f"../../analysis/results_tables/Vgene_usage_{organ}.tsv", sep = " ")
# Unifing subtypes of gene into original one

## DONT DO FOR ALLELES
v["gene"] = [x.split("S")[0] for x in v["gene"]]
if organ == "ILE":
    v["gene"] = [x.split("-")[0] for x in v["gene"]]
...
# {{{
# Read file with frequencies
j = pd.read_csv(f"../../analysis/results_tables/Jgene_usage_{organ}.tsv", sep = " ")
# Unifing subtypes of gene into original one
...
for i in os.listdir("../../analysis/plots/alpha_isotypes/"):
    if os.path.isfile(os.path.join("../../analysis/plots/alpha_isotypes/",i)) and i.startswith("alpha") and i.endswith("{}_isosum.tsv".format(organ)) and "stats" not in i:
        files.append(i)
df = pd.DataFrame()
for e in files:
    s = e[9:]
...
iga = pd.read_csv("../../analysis/results_tables/RDI_IGA_samp_{}_isosum.tsv".format(organ), sep = " ")
igd = pd.read_csv("../../analysis/results_tables/RDI_IGD_samp_{}_isosum.tsv".format(organ), sep = " ")
ige = pd.read_csv("../../analysis/results_tables/RDI_IGE_samp_{}_isosum.tsv".format(organ), sep = " ")
igm = pd.read_csv("../../analysis/results_tables/RDI_IGM_samp_{}_isosum.tsv".format(organ), sep = " ")
igg = pd.read_csv("../../analysis/results_tables/RDI_IGG_samp_{}_isosum.tsv".format(organ), sep = " ")
# We remove the ones that the RDI is == 0 because that is comparison of a sample against itself
...
# {{{
igA = pd.read_csv("../../analysis/plots/clonal_abundance/" + 'clab_IGA_{}.tsv'.format(organ), sep = " ")
igM = pd.read_csv("../../analysis/plots/clonal_abundance/" + 'clab_IGM_{}.tsv'.format(organ), sep = " ")
igD = pd.read_csv("../../analysis/plots/clonal_abundance/" + 'clab_IGD_{}.tsv'.format(organ), sep = " ")
igE = pd.read_csv("../../analysis/plots/clonal_abundance/" + 'clab_IGE_{}.tsv'.format(organ), sep = " ")
igG12 = pd.read_csv("../../analysis/plots/clonal_abundance/" + 'clab_IGG12_{}.tsv'.format(organ), sep = " ")
igG3 = pd.read_csv("../../analysis/plots/clonal_abundance/" + 'clab_IGG3_{}.tsv'.format(organ), sep = " ")
...
shm = pd.read_csv("../../analysis/results_tables/SHM_{}_isotypes_sample_20220412.csv".format(organ), sep = ",")
...
cdr3 = pd.read_csv("../../analysis/results_tables/cdr3_isotypes_{}_20220412.csv".format(organ), sep = ",").drop(columns = "Unnamed: 0")
...
tum_alpha_p20_shm_cdr3_cs_beta_v_j_reliso_alphiso_betiso_p20iso_shmiso_cdr3iso.head()
...
tum_alpha_p20_shm_cdr3_cs_beta_v_j_reliso_alphiso_betiso_p20iso_shmiso_cdr3iso.to_csv("../../analysis/results_tables/tumor_allBCRmetrics_{}_{}.csv".format(organ, new_day), sep = ";")
...
path_mer.to_csv("../../analysis/results_tables/pathology_allBCRmetrics_{}_{}.csv".format(organ, new_day), sep = ";")
