...
for i in os.listdir(path_ori):
    if os.path.isfile(os.path.join(path_ori,i)) and i.startswith("alpha_values") and i.endswith("ILE.tsv"):
        files.append(i)
...
# {{{
AL5 = pd.read_csv(path_ori + 'alpha_values_5AL_ILE.tsv', sep = " ")
DR5 = pd.read_csv(path_ori + 'alpha_values_5DR_ILE.tsv', sep = " ")
AL16 = pd.read_csv(path_ori + 'alpha_values_16AL_ILE.tsv', sep = " ")
AL20 = pd.read_csv(path_ori + 'alpha_values_20AL_ILE.tsv', sep = " ")
DR20 = pd.read_csv(path_ori + 'alpha_values_20DR_ILE.tsv', sep = " ")
AL24 = pd.read_csv(path_ori + 'alpha_values_24AL_ILE.tsv', sep = " ")
DR24 = pd.read_csv(path_ori + 'alpha_values_24DR_ILE.tsv', sep = " ")
ALDR16M20 = pd.read_csv(path_ori + 'alpha_values_20AL_DR16M_ILE.tsv', sep = " ")
ALDR16M24 = pd.read_csv(path_ori + 'alpha_values_24AL_DR16M_ILE.tsv', sep = " ")
ALDR20M24 = pd.read_csv(path_ori + 'alpha_values_24AL_DR20M_ILE.tsv', sep = " ")
...
plt.text(x = -0.12, y = 2000, s = "Richness", rotation = 90, fontsize = 18, color = "black", alpha = 0.5)
plt.text(x = 0.88, y = 2000, s = "Shannon", rotation = 90, fontsize = 18, color = "black", alpha = 0.5)
plt.text(x = 1.88, y = 2000, s = "Simpson", rotation = 90, fontsize = 18, color = "black", alpha = 0.5)

str_vals = [0, 1, 2, 3]
ax.set_xticks([0.0, 1.0, 2.0, 3.0])
ax.set_xticklabels(str_vals)

str_yvals = [0, 1250, 2500, 3750, 5000]
...
plt.tight_layout()
if run_type != "dry":
    plt.savefig("{}ILE_Alpha_Hill_{}.pdf".format(path, new_day))
else:
    plt.show()
# }}}
...
order = ["24M AL_DR16M", "16M AL_lifelong", "20M AL_lifelong", "24M AL_DR20M", "24M AL_lifelong", "20M DR_lifelong", "5M AL_lifelong", "24M DR_lifelong", "5M DR_lifelong", "20M AL_DR16M"]
...
if run_type != "dry":
    plt.savefig("{}ILE_ShannonBox_all_{}.pdf".format(path, new_day))
else:
...
if run_type != "dry":
    plt.savefig("{}ILE_ShannonBox_all_AGE_{}.pdf".format(path, new_day))
else:
...
if run_type == "dry":
    plt.savefig("{}ILE_RichnessBox_all_AGE_{}.pdf".format(path, new_day))
else:
...
def boxplots_cond(df_prov, order, str_vals, text):
    ...
    if run_type != "dry":
        plt.savefig("{}ILE_{}Box_{}_{}.pdf".format(path, text, nam, new_day))
    else:
        plt.show()
...
fig, ax = plt.subplots(figsize = (12,7))
order = ["16M AL_lifelong", "24M AL_DR16M", "20M DR_lifelong", "20M AL_lifelong", "5M AL_lifelong", "5M DR_lifelong", "24M DR_lifelong", "24M AL_DR20M", "24M AL_lifelong", "20M AL_DR16M"]
...
if run_type != "dry":
    plt.savefig("{}ILE_SimpsonBox_all_{}.pdf".format(path, new_day))
else:
...
if run_type == "dry":
    plt.savefig("{}ILE_SimpsonBox_all_AGE_{}.pdf".format(path, new_day))
else:
...
# ## Comparing all biological conditions, age and diet

print(stats.kruskal(shannon_df["d"].to_list(), shannon_df["biogroup"].to_list()))
sp.posthoc_mannwhitney(shannon_df, val_col = "d", group_col = "biogroup")

# ## Comparing Shannon diversity between diets pairwise

anova_lm(ols("d ~ Age * Diet + Age + Diet", data = shannon_df[shannon_df["Diet"].isin(["AL_DR20M", "AL_DR16M"])]).fit(), typ=2)
...
sp.posthoc_conover(sh_5, val_col = "d", group_col = "Diet")
# }}}
...
# ## Comparing simpson diversity anova between diets

anova_lm(ols("d ~ Age * Diet + Age + Diet", data = simpson_df[simpson_df["Diet"].isin(["DR_lifelong", "AL_DR20M"])]).fit(), typ=2)
