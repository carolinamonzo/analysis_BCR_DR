...
#SBATCH --job-name=ILE-CLONES # Job name
#SBATCH --partition=hugemem # Partition
#SBATCH --nodes=1 # Run on one node
#SBATCH --output=./logs/ILE_alpha.log # Standard output and error log
...
ggsave("/beegfs/group_lp/home/CMonzo/CM_IGseq/analysis/plots/Clonal_abundance_ILE.svg", plot = last_plot(), device = NULL, path=NULL, width = 20, height = 10, units = "cm")
...
ggsave("../../analysis/plots/Alpha_Hill_ILE.svg", plot = last_plot(), device = NULL, path=NULL, width = 20, height = 10, units = "cm")

sample_test <- alphaDiversity(db, group="biogroup", clone="clone_id", min_q=0, max_q=2, step_q=1, nboot=200)

write.table(sample_test@tests, file = "../../analysis/plots/alpha_values_stats_ILE.tsv", append=TRUE, col.names = TRUE, row.names = TRUE)
