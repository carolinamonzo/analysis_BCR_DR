...
for i in os.listdir(path):
    if os.path.isfile(os.path.join(path,i)) and i.startswith("alpha") and i.endswith("ILE_isosum.tsv") and "stats" not in i:
        files.append(i)
print(files)
...
for e in ls_biogroup:
    ...
    plt.tight_layout()
    if run_type != "dry":
        plt.savefig("{}ILE_Alpha_ISO-Hill_{}_{}.pdf".format(path, e, new_day))
    else:
        plt.show()
# }}}
ls_biogroup = iso["biogroup"].unique().tolist()
for a in ls_biogroup:
    ...
    plt.tight_layout()
    if run_type != "dry":
        plt.savefig("{}ILE_Alpha_ISO-Hill_Biorep-{}_{}.pdf".format(path, a, new_day))
    else:
        plt.show()
...
for e in ls_age:
    for a in ls_isotype:
        ...
        plt.tight_layout()
        if run_type != "dry":
            plt.savefig("{}ILE_Alpha_ISO-Hill_{}M-{}_{}.pdf".format(path, e, a, new_day))
        else:
            plt.show()
...
def box_alpha_iso(df, text):
    for a in ls_isotype:
        ...
        if run_type == "dry":
            plt.savefig("{}ILE_{}_{}_{}.pdf".format(path, text, a, new_day))
        else:
...
def boxplots_cond(df_prov, order, str_vals, text, a):
    ...
    if run_type == "dry":
        plt.savefig("{}ILE_{}Box_{}_{}_{}.pdf".format(path, text, a, nam, new_day))
        #plt.show()
    else:
        plt.show()
